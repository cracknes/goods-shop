...
- **숫자놀이**: `generateNumberProblem()`이 매번 무작위로 덧셈/뺄셈/곱셈 문제를 만듦. 두 수 모두 **1~9 한 자리 숫자만** 사용 (처음엔 1~99 두 자리, 곱셈만 11×11~19×19 제곱 예외를 넣었었는데, 아이가 아직 두 자리를 어려워해서 전부 한 자리로 낮춤). 뺄셈은 결과가 음수 안 되게 큰 수 - 작은 수로 자동 정렬.
