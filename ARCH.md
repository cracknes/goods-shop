...
지우네 하우스 프로젝트의 세부 구조 (이전 이름: 노르덴돌프). 핵심 요약은 `CLAUDE.md` 참고.
...
| 파일 | 역할 |
|---|---|
| `index.html` | 카테고리(화장품/남성 의류/여성 의류)별 상품 목록 + 마지막 두 탭 "한글놀이"/"숫자놀이"(아이용 미니 게임, DB 연동 없음). 좌측에 카테고리 전용 탭(사이트 공통 상단 nav와는 별개). 상품 카드 클릭 시 상세 팝업이 뜨고, 팝업에 머문 시간을 `product_views`에 기록 (팝업 안의 구매하기 버튼에서 토스 결제 시작) |
...
- 브랜드("지우네 하우스 이지후") 클릭 시 `index.html`로 이동 (헤더에 별도 "상품" 링크는 없음, 로고가 그 역할)
...
- `index.html`의 카테고리 탭 마지막 두 개. DB 저장 없이 순수 클라이언트 로직이고, 정답/오답 시 처리 방식이 공용 함수로 통일되어 있음: 정답이면 `showCelebration(next)` → 하트/이모지 팝업(1.3초, 자동 닫힘) 후 `next()`로 다음 문제 렌더링, 오답이면 `showWrongAnswerAlert()` → `alert("지후야 다시생각해봐")`.
- **한글놀이**: 고정된 단어 목록(`HANGUL_WORDS`)을 하나씩 보여주고 그대로 입력하면 정답.
- **숫자놀이**: `generateNumberProblem()`이 매번 무작위로 덧셈/뺄셈/곱셈 문제를 만듦.
  - 덧셈/뺄셈: 1~99 사이 두 수. 뺄셈은 결과가 음수 안 되게 큰 수 - 작은 수로 자동 정렬.
  - 곱셈: 50% 확률로 (1~9)×(1~9), 나머지 50%는 **11~19 사이 같은 숫자끼리의 제곱**(11×11 ~ 19×19)만 나오게 제한함 (사용자가 명시적으로 요청한 규칙 — 일반 2자리×2자리 곱셈은 안 나옴).
